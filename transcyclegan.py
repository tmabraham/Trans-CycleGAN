...
class TransCycleGAN(nn.Module):
    def __init__(self, image_size = 256, patch_size = 32, dim = 1024, disc_depth = 6, gen_depth = 6, heads = 16, mlp_dim = 3072, ch_in = 3, dim_head = 64, drop=0., emb_drop=0., ):
        super().__init__()
        ...
        self.D_A = DiscriminatorBaseline(image_size=image_size, patch_size=patch_size, dim=dim, depth=disc_depth, heads=heads, mlp_dim=mlp_dim, channels=ch_in, dim_head=dim_head, dropout=drop, emb_dropout=emb_drop)
        self.D_B = DiscriminatorBaseline(image_size=image_size, patch_size=patch_size, dim=dim, depth=disc_depth, heads=heads, mlp_dim=mlp_dim, channels=ch_in, dim_head=dim_head, dropout=drop, emb_dropout=emb_drop)
        self.G_A = GeneratorBaseline(image_size=image_size, patch_size=patch_size,  dim=dim, depth=gen_depth, heads=heads, mlp_dim=mlp_dim, channels=ch_in, dim_head=dim_head, dropout=drop, emb_dropout=emb_drop)
        self.G_B = GeneratorBaseline(image_size=image_size, patch_size=patch_size,  dim=dim, depth=gen_depth, heads=heads, mlp_dim=mlp_dim, channels=ch_in, dim_head=dim_head, dropout=drop, emb_dropout=emb_drop)
